 ...
 * {@link net.bigtangle.net.AbstractTimeoutHandler} and timeout is automatically
 * disabled (using
 * {@link net.bigtangle.net.AbstractTimeoutHandler#setTimeoutEnabled(boolean)})
...
public class Peer extends PeerSocketHandler {
    ...
    private final CopyOnWriteArrayList<ListenerRegistration<BlocksDownloadedEventListener>> blocksDownloadedEventListeners = new CopyOnWriteArrayList<ListenerRegistration<BlocksDownloadedEventListener>>();
    private final CopyOnWriteArrayList<ListenerRegistration<ChainDownloadStartedEventListener>> chainDownloadStartedEventListeners = new CopyOnWriteArrayList<ListenerRegistration<ChainDownloadStartedEventListener>>();
    private final CopyOnWriteArrayList<ListenerRegistration<PeerConnectedEventListener>> connectedEventListeners = new CopyOnWriteArrayList<ListenerRegistration<PeerConnectedEventListener>>();
    private final CopyOnWriteArrayList<ListenerRegistration<PeerDisconnectedEventListener>> disconnectedEventListeners = new CopyOnWriteArrayList<ListenerRegistration<PeerDisconnectedEventListener>>();
    private final CopyOnWriteArrayList<ListenerRegistration<GetDataEventListener>> getDataEventListeners = new CopyOnWriteArrayList<ListenerRegistration<GetDataEventListener>>();
    private final CopyOnWriteArrayList<ListenerRegistration<PreMessageReceivedEventListener>> preMessageReceivedEventListeners = new CopyOnWriteArrayList<ListenerRegistration<PreMessageReceivedEventListener>>();
    private final CopyOnWriteArrayList<ListenerRegistration<OnTransactionBroadcastListener>> onTransactionEventListeners = new CopyOnWriteArrayList<ListenerRegistration<OnTransactionBroadcastListener>>();
     ...
     * or
     * {@link net.bigtangle.net.NioClient#NioClient(java.net.SocketAddress, StreamConnection, int)}.
     * </p>
     *
     * <p>
     ...
     * </p>
     */
    public Peer(NetworkParameters params, VersionMessage ver, @Nullable AbstractBlockGraph chain,
     ...
     * <p>
     * Construct a peer that reads/writes from the given block chain.
     * Transactions stored in a {@link net.bigtangle.core.TxConfidenceTable}
     * will have their confidence levels updated when a peer announces it, to
     * reflect the greater likelyhood that the transaction is valid.
     * </p>
     *
     * <p>
     * Note that this does <b>NOT</b> make a connection to the given
     ...
     * </p>
     */
    public Peer(NetworkParameters params, AbstractBlockGraph blockChain, PeerAddress peerAddress,
            String thisSoftwareName, String thisSoftwareVersion) {
        this(params, new VersionMessage(params, blockChain.getMaxHeight()), blockChain, peerAddress);
    ...
    /**
     * Registers a listener that is called immediately before a message is
     * received
     */
    ...
    public void addPreMessageReceivedEventListener(Executor executor, PreMessageReceivedEventListener listener) {
        preMessageReceivedEventListeners
                .add(new ListenerRegistration<PreMessageReceivedEventListener>(listener, executor));
    }
    ...
    protected void processMessage(Message m) throws Exception {
        ...
        // No further communication is possible until version handshake is
        // complete.
    ...
    private void versionHandshakeComplete() {
        ...
        // call onPeerDisconnected, and we should probably call onPeerConnected
        // first.
    ...
    protected void processAlert(AlertMessage m) {
        try {
            ...
            // https://github.com/bitcoinj/bitcoinj/issues/526 for discussion.
            // As
            // alerts are just optional and not that useful, we just swallow the
            // error here.
    ...
    protected void processHeaders(HeadersMessage m) throws ProtocolException {
        ...
        try {
            ...
            for (int i = 0; i < m.getBlockHeaders().size(); i++) {
                ...
                boolean reachedTop = blockChain.getMaxHeight() >= vPeerVersionMessage.bestHeight;
                if (!passedTime && !reachedTop) {
                    ...
                    try {
                        log.info(
                                ...
                                Utils.dateTimeFormat(fastCatchupTimeSecs * 1000), blockChain.getMaxHeight() + 1,
    ...
    public ListenableFuture<List<Transaction>> downloadDependencies(Transaction tx) {
        // TransactionConfidence.ConfidenceType txConfidence =
        // tx.getConfidence().getConfidenceType();
        // Preconditions.checkArgument(txConfidence !=
    ...
    protected void processBlock(Block m) {
        ...
        try {
            ...
            // This block is an orphan - we don't know how to get from it back
            // to the genesis block yet. That
            // must mean that there are blocks we are missing, so do another
            // getblocks with a new block locator
            // to ask the peer to send them to us. This can happen during the
            // initial block chain download where
            // the peer will only send us 500 at a time and then sends us the
            // head block expecting us to request
            // the others.
            //
            // We must do two things here:
            // (1) Request from current top of chain to the oldest ancestor of
            // the received block in the orphan set
            // (2) Filter out duplicate getblock requests (done in
            // blockChainDownloadLocked).
            //
            // The reason for (1) is that otherwise if new blocks were solved
            // during the middle of chain download
            // we'd do a blockChainDownloadLocked() on the new best chain head,
            // which would cause us to try and grab the
            // chain twice (or more!) on the same connection! The block chain
            // would filter out the duplicates but
            // only at a huge speed penalty. By finding the orphan root we
            // ensure every getblocks looks the same
            // no matter how many blocks are solved, and therefore that the (2)
            // duplicate filtering can work.
            //
            // We only do this if we are not currently downloading headers. If
            // we are then we don't want to kick
            // off a request for lots more headers in parallel.
            lock.lock();
            try {
                if (downloadBlockBodies) {
                    // CUI final Block orphanRoot =
                    // checkNotNull(blockChain.getOrphanRoot(m.getHash()));
    ...
    private void invokeOnBlocksDownloaded(final Block block, @Nullable final FilteredBlock fb) {
        ...
        final long blocksLeft = Math.max(0,
                (long) vPeerVersionMessage.bestHeight - checkNotNull(blockChain).getMaxHeight());
        ...
        try {
            if (blocks.size() > 0 && downloadData && blockChain != null) {
                ...
                for (InventoryItem item : blocks) {
                    // CUI if (blockChain.isOrphan(item.hash) &&
                    // downloadBlockBodies) {
                    ...
                    // CUI final Block orphanRoot =
                    // checkNotNull(blockChain.getOrphanRoot(item.hash));
                    // CUI blockChainDownloadLocked(orphanRoot.getHash());
                    // CUI } else {
                    // Don't re-request blocks we already requested. Normally
                    // this should not happen. However there is
                    // an edge case: if a block is solved and we complete the
                    // inv<->getdata<->block<->getblocks cycle
                    // whilst other parts of the chain are streaming in, then
                    // the new getblocks request won't match the
                    // previous one: whilst the stopHash is the same (because we
                    // use the orphan root), the start hash
                    // will be different and so the getblocks req won't be
                    // dropped as a duplicate. We'll end up
                    // requesting a subset of what we already requested, which
                    // can lead to parallel chain downloads
                    // and other nastyness. So we just do a quick removal of
                    // redundant getdatas here too.
                    //
                    // Note that as of June 2012 Bitcoin Core won't actually
                    // ever interleave blocks pushed as
                    // part of chain download with newly announced blocks, so it
                    // should always be taken care of by
                    // the duplicate check in blockChainDownloadLocked(). But
                    // Bitcoin Core may change in future so
    ...
    public void setDownloadParameters(long secondsSinceEpoch, boolean useFilteredBlocks) {
        ...
        // // If the given time is before the current chains head block time,
        // then this has no effect (we already
        // // downloaded everything we need).
        // if (blockChain != null && fastCatchupTimeSecs >
        // blockChain.getChainHead().getHeader().getTimeSeconds())
    ...
    private void blockChainDownloadLocked(Sha256Hash toHash) {
        ...
        // // The block chain download process is a bit complicated. Basically,
        // we start with one or more blocks in a
        ...
        // // might have got ourselves onto a fork that was later resolved by
        // the network.
        // //
        // // To solve this, we send the peer a block locator which is just a
        // list of block hashes. It contains the
        // // blocks we know about, but not all of them, just enough of them so
        // the peer can figure out if we did end up
        // // on a fork and if so, what the earliest still valid block we know
        // about is likely to be.
        // //
        // // Once it has decided which blocks we need, it will send us an inv
        // with up to 500 block messages. We may
        // // have some of them already if we already have a block chain and
        // just need to catch up. Once we request the
        ...
        // // it to the chain yet because we don't have the intermediate blocks.
        // So we rerun this function building a
        // // new block locator describing where we're up to.
        // //
        // // The getblocks with the new locator gets us another inv with
        // another bunch of blocks. We download them once
        // // again. This time when the peer sends us an inv with the head
        // block, we already have it so we won't download
        // // it again - but we recognize this case as special and call back
        // into blockChainDownloadLocked to continue the
        // // process.
        // //
        // // So this is a complicated process but it has the advantage that we
        // can download a chain of enormous length
        // // in a relatively stateless manner and with constant memory usage.
        // //
        // // All this is made more complicated by the desire to skip
        // downloading the bodies of blocks that pre-date the
        // // 'fast catchup time', which is usually set to the creation date of
        // the earliest key in the wallet. Because
        // // we know there are no transactions using our keys before that date,
        // we need only the headers. To do that we
        // // use the "getheaders" command. Once we find we've gone past the
        // target date, we throw away the downloaded
        ...
        // // 100 block headers. If there is a re-org deeper than that, we'll
        // end up downloading the entire chain. We
        ...
        // log.info("blockChainDownloadLocked({}): ignoring duplicated request:
        // {}", toHash, chainHeadHash);
        ...
        // log.error("Failed to walk the block chain whilst constructing a
        // locator");
        // throw new RuntimeException(e);
        // }
        // }
        // // Only add the locator if we didn't already do so. If the chain is <
        // 50 blocks we already reached it.
        // if (cursor != null)
        ...
        //
        // if (downloadBlockBodies) {
        // GetBlocksMessage message = new GetBlocksMessage(params, blockLocator,
        // toHash);
        // sendMessage(message);
        // } else {
     ...
     * {@link net.bigtangle.store.Peer#getLastPingTime()} is updated.
     * 
     ...
     * {@link net.bigtangle.store.Peer#ping()} has never been called or we did
     * not hear back the "pong" message yet, returns {@link Long#MAX_VALUE}.
     */
     ...
     * {@link net.bigtangle.store.Peer#ping()} has never been called or we did
     * not hear back the "pong" message yet, returns {@link Long#MAX_VALUE}. The
    ...
    public long getPeerBlockHeightDifference() {
        ...
        checkState(params.allowEmptyPeerChain() || chainHeight > 0, "Connected to peer with zero/negative chain height",
                chainHeight);
        return chainHeight - blockChain.getMaxHeight();
    }
