...
public class DeltalakeTest   {
    ...
    private static SparkSession createSession() {
        SparkConf conf = new SparkConf()
                ...
                //.set("spark.sql.warehouse.dir", tmpDir)  only needed for Spark SQL "CREATE DATABASE"
    ...
    public void deltalake() throws IOException, InterruptedException {
        ...
        try {
           ...
           //   table = DeltaTable.forPath(path);
            ...
            Dataset<Row>  deleted= spark.range(90, 103).selectExpr(
                            "CAST(id as INT) as id",
                            "CAST(id as STRING) as name",
                            "CAST(id % 4 + 18 as INT) as age") ;


            Dataset<Row> source =   spark.range(90, 103).selectExpr(
                    "CAST(id as INT) as id",
                    "CAST(id as STRING) as name",
                    "CAST(id % 4 + 18 as INT) as age");
            ...
            table
                    .as("target")
                    .merge(deleted.as("deleted"), "target.id = deleted.id")
                    .whenMatched()
                    .delete()
                    .execute(); // v5
