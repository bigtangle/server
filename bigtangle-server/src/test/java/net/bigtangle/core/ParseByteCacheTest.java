...
public class ParseByteCacheTest {
    ...
    private final byte[] txMessage = HEX.withSeparator(" ", 2).decode("f9 be b4 d9 74 78 00 00  00 00 00 00 00 00 00 00"
    ...
    private final byte[] txMessagePart = HEX.withSeparator(" ", 2)
            .decode("08 5b 1d 8a f7 51 84 f0  bc 01 fa d5 8d 12 66 e9"
    ...
    public void setUp() throws Exception {
        ...
        ;
        TransactionOutput prevOut = new TransactionOutput(PARAMS, prevTx, COIN,
                wallet.currentReceiveKey().toAddress(PARAMS));
    ...
    public void testBlock(byte[] blockBytes, boolean isChild, boolean retain) throws Exception {
        // reference serializer to produce comparison serialization output after
        // changes to
        // message structure.
        ...
        if (b1.getTransactions().size() > 0) {
            ...
            // this will always be true for all children of a block once they
            // are retrieved.
     ...
     //   assertEquals(retain, b1.isTransactionBytesValid());
        ...
        if (b1.getTransactions().size() > 0) {
            ...
            if (tx1.getInputs().size() > 0) {
               ...
               // assertFalse(b1.isTransactionBytesValid());
                ...
                // this has to be false. Altering a tx invalidates the merkle
                // root.
                // when we have seperate merkle caching then the entire header
                // won't need to be
                // invalidated.
           //     assertFalse(b1.isHeaderBytesValid());
        ...
        if (b1.getTransactions().size() > 0) {
            ...
            if (tx1.getInputs().size() > 0) {
                ...
                // chain of fromTx1 so has to have been uncached since it won't
                // be
    ...
    public void testTransaction(NetworkParameters params, byte[] txBytes, boolean isChild, boolean retain)
            throws Exception {
    ...
    private void serDeser(MessageSerializer bs, Message message, byte[] sourceBytes, byte[] containedBytes,
            byte[] containingBytes) throws Exception {
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
