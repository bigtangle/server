...
public class FullPrunedBlockGraph extends AbstractBlockGraph {
	...
	// Whether or not to execute scriptPubKeys before accepting a transaction (i.e.
	// check signatures).
	...
	public TransactionOutputChanges connectTransactions(long height, Block block)
		...
		try {
			if (!params.isCheckpoint(height)) {
				...
				// are all in the
				// checkpoints list and we therefore only check non-checkpoints for duplicated
...
//					final Set<VerifyFlag> verifyFlags = params.getTransactionVerificationFlags(newBlock.getHeader(), tx,
//							getVersionTally());
//					boolean isCoinBase = tx.isCoinBase();
//					Coin valueIn = Coin.ZERO;
//					Coin valueOut = Coin.ZERO;
...
//							prevOutScripts.add(prevOut.getScript());
//
//							blockStore.removeUnspentTransactionOutput(prevOut.getHash(), prevOut.getIndex());
//							txOutsSpent.add(prevOut);
//						}
//					}
//					Sha256Hash hash = tx.getHash();
...
//						valueOut = valueOut.add(out.getValue());
//						Script script = getScript(out.getScriptBytes());
//						UTXO newOut = new UTXO(hash, out.getIndex(), out.getValue(), newBlock.getHeight(), isCoinBase,
...
//	}
	...
	public void disconnectBlock(BlockEvaluation blockEvaluation) throws BlockStoreException {
		...
		for (StoredBlock approver : blockStore.getApproverBlocks(blockEvaluation.getBlockhash())) {
