 ...
 * Creating a new ECKey2 with the empty constructor will generate a new random keypair. Other static methods can be used
 * when you already have the public or private parts. If you create a key with only the public part, you can check
 * signatures but not create them.</p>
 *
 * <p>ECKey2 also provides access to Bitcoin Core compatible text message signing, as accessible via the UI or JSON-RPC.
...
public class ECKey2 implements EncryptableItem {
     ...
     * See the ECKey2 class docs for a discussion of point compression.
     */
    ...
    /**
     * Creates an ECKey2 given the private key only. The public key is calculated from it (this is slow). The resulting
    ...
    /**
     * Creates an ECKey2 given the private key only. The public key is calculated from it (this is slow), either
     * compressed or not.
     */
    ...
    /**
     * Creates an ECKey2 that simply trusts the caller to ensure that point is really the result of multiplying the
    ...
    /**
     * Creates an ECKey2 that cannot be used for signing, only verifying signatures, from the given point. The
     * compression state of pub will be preserved.
     */
    public static ECKey2 fromPublicOnly(ECPoint pub) {
        return new ECKey2(null, pub);
    }

    /**
     * Creates an ECKey2 that cannot be used for signing, only verifying signatures, from the given encoded point.
     * The compression state of pub will be preserved.
     */
    public static ECKey2 fromPublicOnly(byte[] pub) {
        return new ECKey2(null, CURVE.getCurve().decodePoint(pub));
    }
    ...
    public ECKey2 decompress() {
        if (!pub.isCompressed())
            ...
            return new ECKey2(priv, decompressPoint(pub.get()));
    }


    /**
     * Constructs a key that has an encrypted private component. The given object wraps encrypted bytes and an
     * initialization vector. Note that the key will not be decrypted during this call: the returned ECKey2 is
    ...
    /**
     * Output this ECKey2 as an ASN.1 encoded private key, as understood by OpenSSL or used by Bitcoin Core
     * in its wallet storage format.
     * @throws net.bigtangle.core.ECKey2 .MissingPrivateKeyException if the private key is missing or encrypted.
     */
    public byte[] toASN1() {
        try {
    ...
    /**
     * Returns the address that corresponds to the public part of this ECKey2 . Note that an address is derived from
     ...
     * usually encoded using ASN.1 format, so you want {@link net.bigtangle.core.ECKey2 .ECDSASignature#toASN1()}
     * instead. However sometimes the independent components can be useful, for instance, if you're going to do
     * further EC maths on them.
     * @throws KeyCrypterException if this ECKey2 doesn't have a private part.
     */
    ...
    public boolean verify(byte[] hash, byte[] signature) {
        return ECKey2 .verify(hash, signature, getPubKey());
    }
    ...
    public boolean verify(Sha256Hash sigHash, ECDSASignature signature) {
        return ECKey2 .verify(sigHash.getBytes(), signature, getPubKey());
    }
    ...
    public void verifyOrThrow(Sha256Hash sigHash, ECDSASignature signature) throws SignatureException {
        if (!ECKey2 .verify(sigHash.getBytes(), signature, getPubKey()))
            throw new SignatureException();
    }
     ...
     * encoded string.
     *
     * @throws IllegalStateException if this ECKey2 does not have the private part.
     * @throws KeyCrypterException if this ECKey2 is encrypted and no AESKey is provided or it does not decrypt the ECKey2 .
     */
    ...
    public String signMessage(String message, @Nullable KeyParameter aesKey) throws KeyCrypterException {
        ...
        int recId = -1;
        for (int i = 0; i < 4; i++) {
            ECKey2 k = ECKey2 .recoverFromSignature(i, sig, hash, isCompressed());
    ...
    public static ECKey2 signedMessageToKey(String message, String signatureBase64) throws SignatureException {
        ...
        int recId = header - 27;
        ECKey2 key = ECKey2 .recoverFromSignature(recId, sig, messageHash, compressed);
        if (key == null)
    ...
    public void verifyMessage(String message, String signatureBase64) throws SignatureException {
        ECKey2 key = ECKey2 .signedMessageToKey(message, signatureBase64);
     ...
     * @return An ECKey2 containing only the public part, or null if recovery wasn't possible.
     */
    @Nullable
    public static ECKey2 recoverFromSignature(int recId, ECDSASignature sig, Sha256Hash message, boolean compressed) {
        ...
        return ECKey2 .fromPublicOnly(q.getEncoded(compressed));
    }
     ...
     * @throws net.bigtangle.core.ECKey2 .MissingPrivateKeyException if the private key bytes are missing/encrypted.
     */
    ...
    public ECKey2 decrypt(KeyCrypter keyCrypter, KeyParameter aesKey) throws KeyCrypterException {
        ...
        ECKey2 key = ECKey2 .fromPrivate(unencryptedPrivateKey);
        if (!isCompressed())
            key = key.decompress();
    ...
    /**
     * Produce a string rendering of the ECKey2 INCLUDING the private key.
