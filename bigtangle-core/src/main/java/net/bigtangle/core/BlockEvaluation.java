...
public class BlockEvaluation implements Serializable {
	...
	public static BlockEvaluation buildInitial(Block block) {
		...
		return BlockEvaluation.build(block.getHash(), 0, 0, 1, true, 0, false, currentTimeMillis, 0, currentTimeMillis,
				true, false);
	}

	public static BlockEvaluation build(Sha256Hash blockhash, long rating, long depth, long cumulativeWeight,
			boolean solid, long height, boolean milestone, long milestoneLastUpdateTime, long milestoneDepth,
			long insertTime, boolean maintained, boolean validityAssessment) {
	...
	// only relevant for mining reward blocks, true if local assessment deems mining
	// reward block valid
