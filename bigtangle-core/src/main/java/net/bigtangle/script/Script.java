...
public class Script {
	...
	public enum ScriptType {
		// Do NOT change the ordering of the following definitions because their
		// ordinals are stored in databases.
		NO_TYPE, P2PKH, PUB_KEY, P2SH
	}

	/**
	 * Flags to pass to
	 * {@link Script#correctlySpends(Transaction, long, Script, Set)}. Note
	 * currently only P2SH, DERSIG and NULLDUMMY are actually supported.
	 */
	public enum VerifyFlag {
		...
		DERSIG, // Passing a non-strict-DER signature to a checksig operation causes script
				// failure (softfork safe, BIP66 rule 1)
	...
	// Unfortunately, scripts are not ever re-serialized or canonicalized when used
	// in signature hashing. Thus we
	...
	/**
	 * Construct a Script that copies and wraps the programBytes array. The array is
	 * parsed and checked for syntactic validity.
	 * 
	...
	/**
	 * Returns an immutable list of the scripts parsed form. Each chunk is either an
	 * opcode or data element.
	 */
	...
	private static final ScriptChunk[] STANDARD_TRANSACTION_SCRIPT_CHUNKS = {
			new ScriptChunk(ScriptOpCodes.OP_DUP, null, 0), new ScriptChunk(ScriptOpCodes.OP_HASH160, null, 1),
	...
	private void parse(byte[] program) throws ScriptException {
		ByteArrayInputStream bain = new ByteArrayInputStream(program);
		...
		try {
			...
			for (int i = 0; i < size; i++) {
				chunks.add(new ScriptChunk().parse(dis));
			}
		} catch (Exception e) {
			throw new ScriptException("parse", e);
		}
	 ...
	 * representing pushes of data or logical opcodes. Then we can run the parsed
	 * chunks.
	 * </p>
	 *
	 * <p>
	 * The reason for this split, instead of just interpreting directly, is to make
	 * it easier to reach into a programs structure and pull out bits of data
	 * without having to run it. This is necessary to render the to/from addresses
	 * of transactions in a user interface. Bitcoin Core does something similar.
	 * </p>
	 */
	private void parse1(byte[] program) throws ScriptException {
		chunks = new ArrayList<ScriptChunk>(5); // Common size.
		...
		while (bis.available() > 0) {
			...
			if (opcode >= 0 && opcode < OP_PUSHDATA1) {
				...
				// Though this is allowed, because its value cannot be > 520, it should never
				// actually be used
				if (bis.available() < 4)
					throw new ScriptException("Unexpected end of script");
	...
	/**
	 * Returns true if this script is of the form <pubkey> OP_CHECKSIG. This form
	 * was originally intended for transactions where the peers talked to each other
	 * directly via TCP/IP, but has fallen out of favor with time due to that mode
	 * of operation being susceptible to man-in-the-middle attacks. It is still used
	 * in coinbase outputs and can be useful more exotic types of transaction, but
	 * today most payments are to addresses.
	 */
	 ...
	 * 1VayNert3x1KzbpzMGt2qdqrAThiRovi8. This form was originally intended for the
	 * case where you wish to send somebody money with a written code because their
	 * node is offline, but over time has become the standard way to make payments
	 * due to the short and recognizable base58 form addresses come in.
	 */
	 ...
	 * EQUALVERIFY CHECKSIG then this function retrieves the third element. In this
	 * case, this is useful for fetching the destination address of a transaction.
	 * </p>
	 * 
	 * <p>
	 * If a program matches the standard template HASH160 &lt;script hash&gt; EQUAL
	 * then this function retrieves the second element. In this case, this is useful
	 * for fetching the hash of the redeem script of a transaction.
	 * </p>
	 * 
	 * <p>
	...
	/**
	 * Returns the public key in this script. If a script contains two constants and
	 * nothing else, it is assumed to be a scriptSig (input) for a pay-to-address
	 * output and the second constant is returned (the first is the signature). If a
	 * script contains a constant and an OP_CHECKSIG opcode, the constant is
	 * returned as it is assumed to be a direct pay-to-key scriptPubKey (output) and
	 * the first constant is the public key.
	 *
	...
	/**
	 * For 2-element [input] scripts assumes that the paid-to-address can be derived
	 * from the public key. The concept of a "from address" isn't well defined in
	 * Bitcoin and you should not assume the sender of a transaction can actually
	 * receive coins on it. This method may be removed in future.
	 */
	 ...
	 *                         corresponding address. This is useful if you prefer
	...
	/**
	 * Writes out the given byte buffer to the output stream with the correct opcode
	 * prefix To write an integer call writeBytes(out,
	...
	public static byte[] createInputScript(byte[] signature, byte[] pubkey) {
		try {
			// TODO: Do this by creating a Script *first* then having the script reassemble
			// itself into bytes.
	...
	/**
	 * Creates an incomplete scriptSig that, once filled with signatures, can redeem
	 * output containing this scriptPubKey. Instead of the signatures resulting
	 * script has OP_0. Having incomplete input script allows to pass around
	 * partially signed tx. It is expected that this program later on will be
	 * updated with proper signatures.
	 */
	...
	public int getSigInsertionIndex(Sha256Hash hash, ECKey signingKey) {
		// Iterate over existing signatures, skipping the initial OP_0, the final redeem
		// script
	 ...
	 *                         address or is P2SH (run this method on the "Redeem
	 *                         script" instead).
	 */
	...
	/**
	 * Gets the count of regular SigOps in the script program (counting multisig ops
	 * as 20)
	 */
	 ...
	 * ECKey and redeemScript that may be required for certain types of script to
	 ...
	 * output. In such outputs, the logic that controls reclamation is not actually
	 * in the output at all. Instead there's just a hash, and it's up to the
	 * spending input to provide a program matching that hash. This rule is "soft
	 * enforced" by the network as it does not exist in Bitcoin Core. It means
	 * blocks containing P2SH transactions that don't match correctly are considered
	 * valid, but won't be mined upon, so they'll be rapidly re-orgd out of the
	 * chain. This logic is defined by
	 * <a href="https://github.com/bitcoin/bips/blob/master/bip-0016.mediawiki">BIP
	 * 16</a>.
	 * </p>
	 *
	 * <p>
	 * bitcoinj does not support creation of P2SH transactions today. The goal of
	 * P2SH is to allow short addresses even for complex scripts (eg, multi-sig
	 * outputs) so they are convenient to work with in things like QRcodes or with
	 * copy/paste, and also to minimize the size of the unspent output set (which
	 * improves performance of the Bitcoin system).
	 * </p>
	 */
	public boolean isPayToScriptHash() {
		// We have to check against the serialized form because BIP16 defines a P2SH
		// output using an exact byte
		// template, not the logical program structure. Thus you can have two programs
		// that look identical when
	...
	/**
	 * Returns whether this script matches the format used for multisig outputs: [n]
	 * [keys...] [m] CHECKMULTISIG
	 */
	public boolean isSentToMultiSig() {
		...
		if (!(chunk.equalsOpCode(OP_CHECKMULTISIG) || chunk.equalsOpCode(OP_CHECKMULTISIGVERIFY)))
			return false;
		try {
			// Second to last chunk must be an OP_N opcode and there should be that many
			// data chunks (keys).
	...
	/**
	 * Returns the script bytes of inputScript with all instances of the specified
	 * script object removed
	 */
	...
	/*
	 * return the script without chunk signature
	 */
	public  Script removeSignature(  ) { 
		return  removeOpCode(OP_CHECKSIG);
	}
	
	public  Script removeOpCode(  int opCode) {
		List<ScriptChunk> l = new ArrayList<>();
		for(ScriptChunk s:  chunks) {
			if(!s.equalsOpCode(opCode)) {
				l.add(s);
			}
		}
		return new Script(l);
	...
	/**
	 * Returns the script bytes of inputScript with all instances of the given op
	 * code removed
	 */
	public static byte[] removeAllInstancesOfOp(byte[] inputScript, int opCode) {
		return new Script(inputScript).removeOpCode(opCode).getProgram();
				//removeAllInstancesOf(inputScript, new byte[] { (byte) opCode });
	}
	 ...
	 * {@link #castToBigInteger(byte[])} instead, this is only for cases where the
	 * normal maximum length does not apply (i.e. CHECKLOCKTIMEVERIFY).
	 *
	 ...
	 * instead use
	 * {@link net.bigtangle.core.TransactionInput#verify(net.bigtangle.core.TransactionOutput)}
	 * or
	 * {@link net.bigtangle.script.Script#correctlySpends(net.bigtangle.core.Transaction, long, Script)}.
	 * This method is useful if you need more precise control or access to the final
	 * state of the stack. This interface is very likely to change in future.
	 *
	 * @deprecated Use
	 *             {@link #executeScript(net.bigtangle.core.Transaction, long, net.bigtangle.script.Script, java.util.LinkedList, java.util.Set)}
	 *             instead.
	 */
	@Deprecated
	public static void executeScript(@Nullable Transaction txContainingThis, long index, Script script,
		...
		final EnumSet<VerifyFlag> flags = enforceNullDummy ? EnumSet.of(VerifyFlag.NULLDUMMY)
	 ...
	 */
	public static void executeScript(@Nullable Transaction txContainingThis, long index, Script script,
			LinkedList<byte[]> stack, Set<VerifyFlag> verifyFlags) throws ScriptException {
		int opCount = 0;
		...
		for (ScriptChunk chunk : script.chunks) {
			...
			if (chunk.opcode == OP_0) {
				...
				case OP_CHECKLOCKTIMEVERIFY:
					...
					executeCheckLockTimeVerify(txContainingThis, (int) index, script, stack, lastCodeSepLocation,
							opcode, verifyFlags);
					break;
				case OP_NOP1:
				case OP_NOP3:
	...
	private static void executeCheckLockTimeVerify(Transaction txContainingThis, int index, Script script,
		...
		// There are two kinds of nLockTime, need to ensure we're comparing
		// apples-to-apples
	...
	private static void executeCheckSig(Transaction txContainingThis, int index, Script script,
		...
		final boolean requireCanonical = verifyFlags.contains(VerifyFlag.STRICTENC)
				|| verifyFlags.contains(VerifyFlag.DERSIG) || verifyFlags.contains(VerifyFlag.LOW_S);
		if (stack.size() < 2)
		...
		connectedScript = script.removeSignature() .getProgram();
		...
		try {
			...
			// There is (at least) one exception that could be hit here (EOFException, if
			// the sig is too short)
			// Because I can't verify there aren't more, we use a very generic Exception
			// catch

			// This RuntimeException occurs when signing as we run partial/invalid scripts
	...
	private static int executeMultiSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
		...
		while (sigs.size() > 0) {
			...
			// We could reasonably move this out of the loop, but because signature
	...
	/**
	 * Verifies that this script (interpreted as a scriptSig) correctly spends the
	 * given scriptPubKey.
	 * 
	 * @param txContainingThis The transaction in which this input scriptSig
	 *                         resides. Accessing txContainingThis from another
	 *                         thread while this method runs results in undefined
	 *                         behavior.
	 * @param scriptSigIndex   The index in txContainingThis of the scriptSig (note:
	 *                         NOT the index of the scriptPubKey).
	 * @param scriptPubKey     The connected scriptPubKey containing the conditions
	 *                         needed to claim the value.
	 * @param verifyFlags      Each flag enables one validation rule. If in doubt,
	 *                         use
	 *                         {@link #correctlySpends(Transaction, long, Script)}
	...
	public void correctlySpends(Transaction txContainingThis, long scriptSigIndex, Script scriptPubKey,
		...
		// Clone the transaction because executing the script involves editing it, and
		// if we die, we'll leave
		// the tx half broken (also it's not so thread safe to work on it directly.
		try {
			txContainingThis = txContainingThis.getParams().getDefaultSerializer()
					.makeTransaction(txContainingThis.bitcoinSerialize());
		...
		// P2SH is pay to script hash. It means that the scriptPubKey has a special form
		// which is a valid
		// program but it has "useless" form that if evaluated as a normal program
		// always returns true.
		// Instead, miners recognize it as special based on its template - it provides a
		// hash of the real scriptPubKey
		// and that must be provided by the input. The goal of this bizarre arrangement
		// is twofold:
		//
		// (1) You can sum up a large, complex script (like a CHECKMULTISIG script) with
		// an address that's the same
		// size as a regular address. This means it doesn't overload scannable QR
		// codes/NFC tags or become
		// un-wieldy to copy/paste.
		// (2) It allows the working set to be smaller: nodes perform best when they can
		// store as many unspent outputs
		// in RAM as possible, so if the outputs are made smaller and the inputs get
		// bigger, then it's better for
		// overall scalability and performance.

		// TODO: Check if we can take out enforceP2SH if there's a checkpoint at the
		// enforcement block.
