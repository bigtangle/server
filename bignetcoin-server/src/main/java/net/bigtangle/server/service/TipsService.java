...
public class TipsService {
    ...
    public List<Pair<Sha256Hash, Sha256Hash>> getValidatedBlockPairs(int count) throws Exception {
        ...
        for (int index = 0; index < count; index++) {
            ...
            // Find all conflicts
            validatorService.findMilestoneConflicts(approvedNonMilestoneBlocks, conflictingOutPoints,
    ...
    private List<Pair<Sha256Hash, TreeSet<BlockEvaluation>>> getValidatedBlocks(int count, Random seed)
            throws Exception {
    ...
    private List<Sha256Hash> getRatingUpdateEntryPoints(int count, Random seed) throws Exception {
        List<BlockEvaluation> candidates = store.getBlocksInMilestoneDepthInterval(
                NetworkParameters.ENTRYPOINT_RATING_LOWER_DEPTH_CUTOFF,
    ...
    private List<Sha256Hash> getValidationEntryPoints(int count, Random seed) throws Exception {
        List<BlockEvaluation> candidates = store.getBlocksInMilestoneDepthInterval(0,
                NetworkParameters.ENTRYPOINT_VALIDATION_DEPTH_CUTOFF);
        return getRandomsByCumulativeWeight(candidates, count, seed);
    }
    ...
    Sha256Hash randomWalk(Sha256Hash blockHash, final Map<Sha256Hash, Long> cumulativeWeights, Random seed)
        ...
        while (blockHash != null) {
            ...
            if (approverHashes.size() == 0) {
                ...
                long currentCumulativeWeight = cumulativeWeights.containsKey(blockHash)
                        ? cumulativeWeights.get(blockHash)
                        : 1;
                ...
                for (int i = 0; i < blockApprovers.length; i++) {
                    ...
                    transitionWeights[i] = Math.pow(currentCumulativeWeight - cumulativeWeights.get(blockApprovers[i]),
                            -3);
