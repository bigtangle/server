...
public class TransactionRiskAnalysis {
    ...
    private static final Logger log = LoggerFactory.getLogger(TransactionRiskAnalysis.class);

    /**
     * Any standard output smaller than this value (in satoshis) will be
     * considered risky, as it's most likely be rejected by the network. This is
     * usually the same as {@link Transaction#MIN_NONDUST_OUTPUT} but can be
    ...
    public TransactionRiskAnalysis(Transaction tx, List<Transaction> dependencies) {
        this.tx = tx;
    ...
    public Result analyze(int height, long time ) {
        checkState(!analyzed);
        analyzed = true;

        Result result = analyzeIsFinal(height, time);
    ...
    private Result analyzeIsFinal(int height, long time ) {
        ...
        // We consider transactions that opt into replace-by-fee at risk of
        // double spending.
    ...
    /**
     * <p>
     * Checks if a transaction is considered "standard" by Bitcoin Core's
     * IsStandardTx and AreInputsStandard functions.
     * </p>
     *
     * <p>
    ...
    /**
     * Checks the output to see if the script violates a standardness rule. Not
     * complete.
     */
